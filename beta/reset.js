...
function expand(override) {
    ...
    if (
        ...
        let exotic_count = 0
        for (let i = 0; i < 7; i++) {
            game.exotic_shards[i] += game.pending_shards[i]
            exotic_count += game.pending_shards[i]
        }
        if (game.pending_shards[5] >= 1 && !game.rainbow_shards)
            game.rainbow_shards = true
        if (game.pending_shards[6] >= 1 && !game.dark_shards)
            game.dark_shards = true
        ...
        for (let i = 8; i >= 0; i--) {
            ...
            game.expand_exotic_history[i + 1] = game.expand_exotic_history[i]
        ...
        game.expand_exotic_history[0] = exotic_count
        ...
        if (!game.realms_visited.includes(game.selected_realm)) {
            ...
            let current = document
                ...
                .querySelectorAll(".realm")[game.current_realm]
            current.className = "realm visited_realm"
            if (game.selected_realm === game.current_realm)
                current.className = "realm visited_realm current_realm"
            current.style.padding =
                0.5 * realm.realms[game.current_realm].size + "em"
        }
        game.previous_realm = game.current_realm
        ...
        let previous = document
            .getElementById("exploration_map")
            .querySelectorAll(".realm")[game.previous_realm]
        previous.className = "realm visited_realm"
        if (game.current_realm === game.previous_realm)
            previous.className = "realm visited_realm current_realm"
        previous.style.padding =
            0.5 * realm.realms[game.previous_realm].size + "em"

        if (
            game.realms_visited.includes(game.current_realm) &&
            (game.realms_visited[game.realms_visited.length - 1] !==
                game.current_realm ||
                game.previous_realm === game.current_realm)
        ) {
            document
                .getElementById("exploration_map")
                .querySelectorAll(".realm")[game.current_realm].className =
                "realm visited_realm current_realm"
            document
                .getElementById("exploration_map")
                .querySelectorAll(".realm")[game.current_realm].style.padding =
                0.5 * realm.realms[game.previous_realm].size + "em"
        } else {
            document
                .getElementById("exploration_map")
        ...
        if (
            ...
            let next = realm.realms[game.autoex_queue[0]]
            let target = realm.realms[game.target_realm]

            if (
                ((next.x ** 2 + next.y ** 2) ** 0.5 <= 160 &&
                    crystals_equipped === 0) ||
                next.id < 6
            ) {
                game.target_realm = -1
                ...
                if (game.selected_realm === game.target_realm) {
        ...
        if (
            game.autoex_mode === 1 &&
            game.current_realm === game.target_realm
        ) {
