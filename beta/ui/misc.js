...
function stats_update() {
    ...
    for (let i = 0; i < 10; i++) {
        if (
            game.expand_time_history[i] === -1 ||
            (game.statistics_unit[3] === 1 &&
                game.expand_exotic_history[i] === -1)
        ) {
            stats_str += "<br>#" + (i + 1) + " (no data)"
        } else {
            entries++
            ...
            average5 += game.expand_exotic_history[i] / time
            stats_str += "<br>#" + (i + 1)
            stats_str += " took " + format_time(time, game.notation, true)
            if (game.statistics_time === 1) stats_str += " real time"
            if (game.statistics_unit[3] === 0) {
                ...
                if (game.expand_exotic_history[i] === 1)
                    stats_str +=
                        " and gave " +
                        format_num(
                            game.expand_exotic_history[i],
                            game.notation
                        ) +
                        " exotic shard."
                else
                    stats_str +=
                        ...
                        " exotic shards."
                if ((game.expand_exotic_history[i] * 60) / time >= 1)
                    stats_str +=
                        " +" +
                        format_dec(
                            (game.expand_exotic_history[i] * 60) / time,
                            game.notation
                        ) +
                        " exotic shards/min"
                else
                    stats_str +=
                        " +" +
                        format_dec(
                            (game.expand_exotic_history[i] * 3600) / time,
                            game.notation
                        ) +
                        " exotic shards/hour"
            } else {
    ...
    if (entries > 0) {
        ...
        if (game.statistics_unit[3] === 0) {
            ...
            if (average5 * 60 >= 1)
                stats_str +=
                    "<br><br>Average exotic shard gain: +" +
                    format_dec(average5 * 60, game.notation) +
                    " exotic shards/min"
            else
                stats_str +=
                    "<br><br>Average exotic shard gain: +" +
                    format_dec(average5 * 3600, game.notation) +
                    " exotic shards/hour"
        } else {
            if (average2 * 60 >= 1) {
                if (average2 * 60 >= 1000000)
        ...
        else if (game.statistics_unit[3] === 0)
            stats_str +=
                "<br><br>Average exotic shard gain: undefined<br>Average time: undefined"
        else
            stats_str +=
