    ...
    switch (color) {
        ...
        case "expand":
            if (game.autoex_toggle) {
                ...
                document.getElementById("expand_auto_toggle").innerHTML =
                    "Auto: OFF"
                document.getElementById("expand_auto_toggle").className =
                    "spice_buy a_disabled"
            } else {
                if (!game.expand_confirm || unless) {
                    game.autoex_toggle = true
                    document.getElementById("expand_auto_toggle").innerHTML =
                        "Auto: ON"
                    document.getElementById("expand_auto_toggle").className =
                        "spice_buy a_enabled"
                    if (game.autoex_mode === 1) {
                        let selection = undefined
                        let selection_distance = undefined
                        let current = realm.realms[game.current_realm]
                        for (let i = 6; i < realm.realms.length; i++) {
                            if (current.id !== i) {
                                if (selection !== undefined) {
                                    let candidate = realm.realms[i]
                                    let candidate_distance =
                                        (candidate.x ** 2 + candidate.y ** 2) *
                                        ((candidate.x - current.x) ** 2 +
                                            (candidate.y - current.y) ** 2)
                                    if (
                                        !game.realms_visited.includes(i) &&
                                        candidate_distance < selection_distance
                                    ) {
                                        selection = candidate
                                        selection_distance = candidate_distance
                                    }
                                } else {
                                    if (!game.realms_visited.includes(i)) {
                                        selection = realm.realms[i]
                                        selection_distance =
                                            (selection.x ** 2 +
                                                selection.y ** 2) *
                                            ((selection.x - current.x) ** 2 +
                                                (selection.y - current.y) ** 2)
                                    }
                                }
                            }
                        }
                        if (selection !== undefined) {
                            target_realm(selection)
                        } else {
                            game.target_realm = -1
                    ...
                    if (game.target_realm !== -1) {
                        select_realm(realm.realms[game.autoex_queue[0]])
                        game.autoex_queue.shift()
                    }
                } else {
                    if (modal === "none") {
                        open_modal(
                            "alert",
                            "Expansion confirmations must be off to turn on Expansion automation!",
                        )
                    }
                }
            }
            ...
            if (game.autoex_mode === 0) {
                ...
                if (game.autoex_toggle) {
                    ...
                    if (selection !== undefined) {
                        ...
                        open_modal(
                            "alert",
                            "Auto-Expansion targeting mode can't be switched to UNVISITED, as there are no more unvisited realms!",
                        )
                    }
                }
    ...
    if (max) {
            ...
            if (game.ascend_bought[5]) {
                if (
                    ...
                    if (
                        ...
                        if (game.prestige_bought[id] < 1100000) {
                            let n = Math.floor(
                                5.5 +
                                    (game.rainbow_spice.ln() -
                                        492 * Math.log(2)) **
                                        0.5 /
                                        (2 * (2 * Math.log(2)) ** 0.5),
                            )
                            if (n > 1100000) n = 1100000
                            let price = Decimal.pow(
                                2,
                                8 * n ** 2 - 88 * n + 734,
                            )
                            ...
                            if (n > game.prestige_bought[id]) {
                                ...
                                if (game.prestige_bought[id] < 1100000)
                                    ...
                                    prestige_upgrade.upgrades[id].price =
                                        Decimal.pow(
                                            2,
                                            (8 * game.prestige_bought[id] ** 2 -
                                                72 * game.prestige_bought[id] +
                                                654) *
                                                ((game.prestige_bought[id] +
                                                    1) /
                                                    1100000) **
                        ...
                        if (
                            game.prestige_bought[id] >= 1100000 &&
                            game.rainbow_spice.cmp(
                                prestige_upgrade.upgrades[id].price,
                            ) >= 0
                        ) {
                            let l = game.rainbow_spice.log(2)
                            ...
                            for (let i = 0; i < 100; i++) {
                                ...
                                if (Math.abs(f / fp) < 1e-14) break
                                if (Math.abs(n - n_prev) < 1e-14) break
                            }
        ...
        if (id === 20) {
                ...
                if (
                    ...
                    if (
                        ...
                        if (game.prestige_bought[id] < 1100000) {
                            ...
                            prestige_upgrade.upgrades[id].price = Decimal.pow(
                                2,
                                (8 * game.prestige_bought[id] ** 2 -
                                    72 * game.prestige_bought[id] +
                                    654) *
            ...
            if (
                ...
                if (id === 12 && !entry_unlocked[5]) entry_unlock(5)
...
function buy_infusion() {
    if (
        ...
        if (game.crystal_infusion >= 55n * 10n ** 16n) {
            game.crystal_infusion_price = game.crystal_infusion_price.mul(
                Decimal.pow(
                    2.304e12,
                    Number(game.crystal_infusion) / 5.5e17 - 1,
                ),
            )
        }
...
function max_infusion() {
    ...
    if (
        ...
        if (game.crystal_infusion < 55n * 10n ** 16n) {
            ...
            if (game.crystal_infusion + n >= 55n * 10n ** 16n)
                n = 55n * 10n ** 16n - game.crystal_infusion
            if (n > 0n) {
                let price = game.crystal_infusion_price
                    .mul(
                        new Decimal(1).sub(
        ...
        if (game.crystal_infusion >= 55n * 10n ** 16n) {
            if (
                game.crystal_infusion < 709810864567609503n &&
                game.crystal_spice.cmp(0) === 1
            ) {
                let price = game.crystal_infusion_price.mul(
                    Decimal.pow(
                        2.304e12,
                        Number(game.crystal_infusion) / 5.5e17,
                    ),
                )
                if (
                    game.crystal_spice.cmp(
                        game.crystal_infusion_price.add(price),
                    ) >= 0
                ) {
                    let l =
                        game.crystal_spice
                            .div(game.crystal_infusion_price)
                            .ln() / Math.log(2.304e12)
                    let n =
                        1n +
                        BigInt(
                            Math.max(
                                -1,
                                Math.floor(
                                    ((Number(game.crystal_infusion) - 0.5) **
                                        2 +
                                        1.1e18 * l) **
                                        0.5 -
                                        Number(game.crystal_infusion) +
                                        0.5,
                                ),
                            ),
                        )
                    let price2 = game.crystal_infusion_price.mul(
                        Decimal.pow(
                            2.304e12,
                            Number(
                                game.crystal_infusion * (n - 1n) +
                                    ((n - 1n) * (n - 2n)) / 2n,
                            ) / 5.5e17,
                        ),
                    )
                    let price3 = game.crystal_infusion_price.mul(
                        Decimal.pow(
                            2.304e12,
                            Number(
                                game.crystal_infusion * (n - 2n) +
                                    ((n - 2n) * (n - 3n)) / 2n,
                            ) / 5.5e17,
                        ),
                    )
                    price2 = price2.add(price3)
                    if (game.crystal_spice.cmp(price2) === -1) {
                        n -= 1n
                        price2 = price3.add(
                            game.crystal_infusion_price.mul(
                                Decimal.pow(
                                    2.304e12,
                                    Number(
                                        game.crystal_infusion * (n - 2n) +
                    ...
                    if (game.crystal_infusion + n >= 709810864567609503n)
                        n = 709810864567609503n - game.crystal_infusion
                    if (n > 0n) {
                        game.crystal_spice = Decimal.max(
                            game.crystal_spice.sub(price2),
                            0,
                        )
                        game.crystal_infusion_price =
                            game.crystal_infusion_price.mul(
                                Decimal.pow(
                                    2.304e12,
                                    Number(
            ...
            if (game.crystal_spice.cmp(0) === 1) {
                let l =
                    game.crystal_spice.div(game.crystal_infusion_price).ln() /
                    Math.log(2.304e12)
                let n =
                    1n +
                    BigInt(
                        Math.max(
                            -1,
                            Math.floor(
                                ((Number(game.crystal_infusion) - 0.5) ** 2 +
                                    1.1e18 * l) **
                                    0.5 -
                                    Number(game.crystal_infusion) +
                                    0.5,
...
function buy_enchantment() {
    if (game.arcane_spice.cmp(game.arcane_enchantment_price) >= 0) {
        ...
        if (game.arcane_enchantment >= 77n * 10n ** 13n)
            game.arcane_enchantment_price = game.arcane_enchantment_price.mul(
                Decimal.pow(
                    3628800,
                    Number(game.arcane_enchantment) / 7.7e14 - 1,
                ),
            )
    ...
    if (game.arcane_spice.cmp(game.arcane_enchantment_price) >= 0) {
        ...
        if (game.ascend_challenge === 5 || game.collapse_challenge === 7) {
            ...
            if (game.arcane_enchantment < 77n * 10n ** 13n) {
                ...
                if (game.arcane_enchantment + n >= 77n * 10n ** 13n)
                    n = 77n * 10n ** 13n - game.arcane_enchantment
                if (n > 0n) {
                    let price = game.arcane_enchantment_price
                        .mul(
                            new Decimal(1).sub(
            ...
            if (game.arcane_enchantment >= 77n * 10n ** 13n) {
                if (
                    game.arcane_enchantment < 936393133522954n &&
                    game.arcane_spice.cmp(0) === 1
                ) {
                    let price = game.arcane_enchantment_price.mul(
                        Decimal.pow(
                            3628800,
                            Number(game.arcane_enchantment) / 7.7e14,
                        ),
                    )
                    let price2 = game.arcane_enchantment_price.mul(
                        Decimal.pow(
                            3628800,
                            Number(game.arcane_enchantment * 2n + 1n) / 7.7e14,
                        ),
                    )
                    if (
                        game.arcane_spice.cmp(
                            game.arcane_enchantment_price
                                .add(price)
                                .add(price2),
                        ) >= 0
                        ...
                        let n =
                            ...
                            BigInt(
                                Math.max(
                                    -1,
                                    Math.floor(
                                        ((Number(game.arcane_enchantment) -
                                            0.5) **
                                            2 +
                                            1.54e15 * l) **
                                            0.5 -
                        ...
                        let price3 = game.arcane_enchantment_price.mul(
                            Decimal.pow(
                                3628800,
                                Number(
                                    game.arcane_enchantment * (n - 1n) +
                                        ((n - 1n) * (n - 2n)) / 2n,
                                ) / 7.7e14,
                            ),
                        )
                        let price4 = game.arcane_enchantment_price.mul(
                            Decimal.pow(
                                3628800,
                                Number(
                                    game.arcane_enchantment * (n - 2n) +
                                        ((n - 2n) * (n - 3n)) / 2n,
                                ) / 7.7e14,
                            ),
                        )
                        let price5 = game.arcane_enchantment_price.mul(
                            Decimal.pow(
                                3628800,
                                Number(
                                    game.arcane_enchantment * (n - 3n) +
                                        ((n - 3n) * (n - 4n)) / 2n,
                                ) / 7.7e14,
                            ),
                        )
                        price3 = price3.add(price4).add(price5)
                        if (game.arcane_spice.cmp(price3) === -1) {
                            n -= 1n
                            price3 = price4
                                .add(price5)
                                .add(
                        ...
                        if (game.arcane_enchantment + n >= 936393133522954n)
                            n = 936393133522954n - game.arcane_enchantment
                        if (n > 0n) {
                            game.arcane_spice = Decimal.max(
                                game.arcane_spice.sub(price3),
                                0,
                            )
                            game.arcane_enchantment_price =
                                game.arcane_enchantment_price.mul(
                        ...
                        game.arcane_spice.cmp(
                            game.arcane_enchantment_price.add(price),
                        ) >= 0
                    ) {
                        buy_enchantment()
                        buy_enchantment()
                    } else if (
                        game.arcane_spice.cmp(game.arcane_enchantment_price) >=
                        0
                    ) {
                        buy_enchantment()
                    }
                ...
                if (
                    game.arcane_enchantment < 1872786267045908n &&
                    game.arcane_spice.cmp(0) === 1
                ) {
                    let price = game.arcane_enchantment_price.mul(
                        Decimal.pow(
                    ...
                    if (
                        ...
                        price2 = price2.add(price3)
                        if (game.arcane_spice.cmp(price2) === -1) {
                            n -= 1n
                            price2 = price3.add(
                                game.arcane_enchantment_price.mul(
                                    Decimal.pow(
                        ...
                        if (game.arcane_enchantment + n >= 1872786267045908n)
                            n = 1872786267045908n - game.arcane_enchantment
                        if (n > 0n) {
                            game.arcane_spice = Decimal.max(
                                game.arcane_spice.sub(price2),
                                0,
                            )
                            game.arcane_enchantment_price =
                                game.arcane_enchantment_price.mul(
                    ...
                    let n =
                        ...
                        BigInt(
                            Math.max(
                                -1,
                                Math.floor(
                                    ((Number(game.arcane_enchantment) - 0.5) **
                                        2 +
                                        1.54e15 * l) **
                                        0.5 -
                                        Number(game.arcane_enchantment) +
...
//selecting realms
function select_realm(r) {
    if (game.selected_realm === r.id) {
        ...
        if (
            ((r.x - realm.realms[game.current_realm].x) ** 2 +
                (r.y - realm.realms[game.current_realm].y) ** 2) **
                0.5 <
                40 + 10 * game.jump_distance_level &&
            ((r.x ** 2 + r.y ** 2) ** 0.5 > 160 || crystals_equipped >= 1) &&
            r.id >= 6
        ) {
            ...
            document.getElementById("exploration_selected").style.left =
                1020 + 0.6 * r.x - 1.875 * r.size + "em"
            document.getElementById("exploration_selected").style.top =
                1020 + 0.6 * r.y - 1.875 * r.size + "em"
            if (r.id === game.current_realm)
...
//choosing a target realm
function target_realm(r) {
    if (game.galactic_bought[25]) {
        if (game.target_realm === r.id) {
            game.target_realm = -1
            ...
            if (
                ((r.x ** 2 + r.y ** 2) ** 0.5 > 160 ||
                    crystals_equipped >= 1) &&
                r.id >= 6 &&
                r.id !== game.current_realm
            ) {
                game.target_realm = r.id
                game.autoex_queue = calculate_path(
                    realm.realms[game.current_realm],
                    r,
                )
                game.autoex_queue.shift()
                if (game.autoex_toggle) {
                ...
                document.getElementById("exploration_target").style.display =
                    "block"
...
//decrementing a recipe item
function recipe_down(item) {
    switch (item) {
        case "count":
            if (game.recipe_change > 1) game.recipe_change /= 10
            break
        case "type":
            if (game.recipe[0] > 0) game.recipe[0]--
            if (game.recipe[1] > game.exotic_shards[game.recipe[0]])
                game.recipe[1] = game.exotic_shards[game.recipe[0]]
            break
        case "level":
            game.crystal_level = Math.max(
                game.crystal_level - game.recipe_change,
                1,
            )
            break
        case "shards":
            game.recipe[1] = Math.max(game.recipe[1] - game.recipe_change, 0)
            break
    }
...
//incrementing a recipe item
function recipe_up(item) {
    switch (item) {
        case "count":
            let highest_shards = 0
            for (let i = 0; i < 7; i++) {
                if (game.exotic_shards[i] > highest_shards)
                    highest_shards = game.exotic_shards[i]
            }
            if (game.recipe_change < highest_shards) game.recipe_change *= 10
            break
        case "type":
            if (game.recipe[0] < 6 && game.dark_shards) game.recipe[0]++
            else if (game.recipe[0] < 5 && game.rainbow_shards) game.recipe[0]++
            else if (game.recipe[0] < 4) game.recipe[0]++
            if (game.recipe[1] > game.exotic_shards[game.recipe[0]])
                game.recipe[1] = game.exotic_shards[game.recipe[0]]
            break
        case "level":
            game.crystal_level += game.recipe_change
            if (game.crystal_level === Infinity)
                game.crystal_level = Number.MAX_VALUE
            break
        case "shards":
            game.recipe[1] = Math.min(
                game.recipe[1] + game.recipe_change,
                game.exotic_shards[game.recipe[0]],
            )
            break
    }
...
//randomizing galactic crystal effects when changing the recipe
function recipe_randomize() {
    game.preview = {
        new: true,
        level: game.crystal_level,
        value: new Decimal(0),
        type: game.recipe[0],
        essence: Math.min(get_essence(game.recipe[1], game.recipe[0]), 100),
        effects: [],
        recipe: game.recipe[1],
    }
    let count = 1
    if (game.preview.essence >= 5) count++
    if (game.preview.type >= 5) count = 2
    if (game.preview.essence >= 37.5) count++
    let unlocked = 3
    if (game.preview.essence >= 25) unlocked++
    if (game.preview.essence >= 50) unlocked++
    if (game.preview.essence >= 75 && game.preview.type >= 5) unlocked++
    if (count > unlocked) count = unlocked
    let options
    switch (count) {
        case 1:
            options = [[0], [1], [2]]
    ...
    let list = [game.recipe[0], Math.floor(game.recipe[1] / options.length)]
    let str = btoa(JSON.stringify(list))
    let length = Math.floor(str.length / 4)
    let crystal_seed = new Int32Array(4)
    for (let i = 0; i < 3; i++) {
        crystal_seed[i] = str.slice(0, length).hashCode()
        str = str.slice(length)
    }
    crystal_seed[3] = str.hashCode()
    for (let i = 0; i < 4; i++) {
        crystal_seed[i] ^= game.seed[i]
    }
    random_seed = crystal_seed
    for (let i = 0; i < 10; i++) {
        random_float()
    }
    if (options.length > 1) {
        ...
        while (c !== 0) {
            ...
            c--
            ;[options[c], options[r]] = [options[r], options[c]]
        }
        let choice = game.recipe[1] % options.length
        for (let i = 0; i < options[choice].length; i++) {
            game.preview.effects.push(options[choice][i])
        }
    } else {
        for (let i = 0; i < options[0].length; i++) {
            game.preview.effects.push(options[0][i])
        }
    }
}

//crafting the crystal in the preview
function craft() {
    ...
    let mult = Decimal.pow(
        ...
        6.898 +
            17.1 *
                (Math.max(Math.log(game.recipe[1]), 0) / Math.log(6e14)) ** 2,
    ).pow(shard_power[game.recipe[0]])
    price = price.mul(mult).mul(shard_mul[game.recipe[0]])
    if (game.galactic_shards.cmp(price) >= 0) {
        game.galactic_shards = Decimal.max(0, game.galactic_shards.sub(price))
        game.preview.value = price.div(2)
        game.preview.recipe = game.recipe[1]
        for (let i = 0; i < 30; i++) {
            if (game.inventory[i] === null) {
                game.inventory[i] = game.preview
                update_slot(i)
                break
            }
        }
        game.exotic_shards[game.recipe[0]] -= game.recipe[1]
        if (game.recipe[1] > game.exotic_shards[game.recipe[0]])
            game.recipe[1] = game.exotic_shards[game.recipe[0]]
        recipe_randomize()
    }
...
//clearing out selected galactic crystal inventory slots
function deselect_slots() {
    if (game.selected_slot[0] >= 30)
        document.getElementById("slot" + game.selected_slot[0]).className =
            "inventory_slot equip_slot"
    else if (game.selected_slot[0] >= 0)
        document.getElementById("slot" + game.selected_slot[0]).className =
            "inventory_slot"
    if (game.selected_slot[1] >= 30)
        document.getElementById("slot" + game.selected_slot[1]).className =
            "inventory_slot equip_slot"
    else if (game.selected_slot[1] >= 0)
        document.getElementById("slot" + game.selected_slot[1]).className =
            "inventory_slot"
...
//moving a galactic crystal to another slot
function move_crystal() {
    if (game.selected_slot[0] >= 30) {
        game.equip[game.selected_slot[1] - 30] =
            game.equip[game.selected_slot[0] - 30]
        game.equip[game.selected_slot[0] - 30] = null
    } else {
        game.inventory[game.selected_slot[1]] =
            game.inventory[game.selected_slot[0]]
        game.inventory[game.selected_slot[0]] = null
    }

    let temp = game.selected_slot[0]
    game.selected_slot[0] = game.selected_slot[1]
    game.selected_slot[1] = temp

    update_slot(game.selected_slot[0])
    update_slot(game.selected_slot[1])
    deselect_slots()
}

//swapping two galactic crystals
function swap_crystals() {
    if (game.selected_slot[0] >= 30 || game.selected_slot[1] >= 30) {
        if (modal === "none") {
            open_modal(
                "confirm",
                "Are you sure you want to swap these galactic crystals? You will Expand!",
                function () {
                    if (
                        game.selected_slot[0] >= 30 &&
                        game.selected_slot[1] >= 30
                    ) {
                        let temp = game.equip[game.selected_slot[0] - 30]
                        game.equip[game.selected_slot[0] - 30] =
                            game.equip[game.selected_slot[1] - 30]
                        game.equip[game.selected_slot[1] - 30] = temp
                    } else if (game.selected_slot[0] >= 30) {
                        let temp = game.equip[game.selected_slot[0] - 30]
                        game.equip[game.selected_slot[0] - 30] =
                            game.inventory[game.selected_slot[1]]
                        game.inventory[game.selected_slot[1]] = temp
                    } else if (game.selected_slot[1] >= 30) {
                        let temp = game.inventory[game.selected_slot[0]]
                        game.inventory[game.selected_slot[0]] =
                            game.equip[game.selected_slot[1] - 30]
                        game.equip[game.selected_slot[1] - 30] = temp
                    }
...
//scrapping a galactic crystal
function scrap_crystal() {
    if (game.selected_slot[0] >= 30) {
        if (modal === "none") {
            open_modal(
                "confirm",
                "Are you sure you want to scrap this equipped galactic crystal? You will gain " +
                    format_inum(
                        game.equip[game.selected_slot[0] - 30].value,
                        game.notation,
                    ) +
                    " galactic shards and also Expand!",
                function () {
                    game.galactic_shards = game.galactic_shards.add(
                        game.equip[game.selected_slot[0] - 30].value,
                    )
                    game.equip[game.selected_slot[0] - 30] = null
        ...
        if (modal === "none") {
            open_modal(
                "confirm",
                "Are you sure you want to scrap this galactic crystal? You will gain " +
                    format_inum(
                        game.inventory[game.selected_slot[0]].value,
                        game.notation,
                    ) +
                    " galactic shards",
                function () {
...
//equipping a galactic crystal
function equip_crystal() {
    if (game.selected_slot[1] === -1) {
        if (game.equip[0] === null) {
            game.equip[0] = game.inventory[game.selected_slot[0]]
            game.inventory[game.selected_slot[0]] = null
            ...
            deselect_slots()
            update_crystal_boosts()
        } else if (game.equip[1] === null) {
            game.equip[1] = game.inventory[game.selected_slot[0]]
            game.inventory[game.selected_slot[0]] = null
            ...
            deselect_slots()
            update_crystal_boosts()
        } else if (game.equip[2] === null && game.galactic_bought[24]) {
            game.equip[2] = game.inventory[game.selected_slot[0]]
            game.inventory[game.selected_slot[0]] = null
...
//unequpping a galactic crystal
function unequip_crystal() {
    if (game.selected_slot[1] === -1) {
        let free_slot = -1
        for (let i = 0; i < 30; i++) {
            if (game.inventory[i] === null) {
        ...
        if (free_slot !== -1) {
            if (modal === "none") {
                open_modal(
                    "confirm",
                    "Are you sure you want to unequip this galactic crystal? You will Expand!",
                    function () {
                        game.inventory[free_slot] =
                            game.equip[game.selected_slot[0] - 30]
                        game.equip[game.selected_slot[0] - 30] = null
...
//updating galactic crystal boosts
function update_crystal_boosts() {
    for (let i = 0; i < 5; i++) {
        game.crystal_boost[i][0] = new Decimal(1)
        ...
        switch (i) {
            ...
            case 3:
                ...
                break
            case 4:
                game.crystal_boost[i][2] = new Decimal(1)
                break
        }
        ...
        if (crystal !== null) {
            if (crystal.new) {
                for (let j = 0; j < crystal.effects.length; j++) {
                    let power =
                        crystal.essence +
                        crystal.essence ** 2 / 100 +
                        (crystal.level - 1) / 2
                    let power2 = power
                    if (power > 300)
                        power = 10.30875 * Math.exp(0.0131328 * power) + 763.361
                    else power = power + power ** 1.8 / 28.9551
                    /*power = power * Math.exp(0.000363494 * power ** 1.455)*/
                    let scale2 = 1
                    let scale3
                    switch (crystal.effects.length) {
                        case 1:
                            scale2 = 1.25
                            break
                        case 2:
                        ...
                        case 0:
                            switch (crystal.type) {
                                case 5:
                                    ...
                                    break
                                default:
                                    game.crystal_stack[crystal.type][0].push(
                                        Decimal.pow(
                                            10,
                                            3.442835e18 *
                                                power *
                        ...
                        case 1:
                            switch (crystal.type) {
                                case 5:
                                    ...
                                    break
                                default:
                                    scale3 = [1, 0.951, 0.913, 0.883, 0.852]
                                    game.crystal_stack[crystal.type][1].push(
                                        power ** 0.5376 *
                                            4999 *
                                            scale *
                                            scale2 *
                        ...
                        case 2:
                            switch (crystal.type) {
                                ...
                                case 2:
                                    ...
                                    break
                                case 3:
                                    game.crystal_stack[3][2].push(
                                        (power + 1) ** (28.5 * scale * scale2),
                                    )
                                    break
                                case 4:
                                    game.crystal_stack[4][2].push(
                                        Decimal.pow(
                                            10,
                                            3.442835e18 *
                                                power *
                                    ...
                                    break
                                case 5:
                                    game.crystal_stack[5][2].push(
                                        ((power2 / 1225) ** 0.395 * 1.21193 +
                                            0.288072) *
                                            scale *
                                            scale2 *
                                            1.058,
...
//hiding the exploration ui
function exploration_hide() {
    if (!game.exploration_hidden) {
        game.exploration_hidden = true
        document.getElementById("exploration_hide").innerHTML = "Show"
        document.getElementById("expansion_panel").style.display = "none"
    } else {
        game.exploration_hidden = false
        document.getElementById("exploration_hide").innerHTML = "Hide"
        document.getElementById("expansion_panel").style.display = "block"
    }
}

//resetting the zoom and scroll on the exploration screen
function exploration_reset() {
    zoom_level = 1
    exploration_moved = false
...
//zooming the exploration screen in or out
function exploration_zoom(direction) {
    if (direction === "out") {
        zoom_level *= 1.5
        if (zoom_level > 11.390625) zoom_level = 11.390625
    } else if (direction === "in") {
        zoom_level /= 1.5
        if (zoom_level < 1) zoom_level = 1
    }
    exploration_moved = true
    ...
    screen_width = document.getElementById("exploration_view").clientWidth
    screen_height = document.getElementById("exploration_view").clientHeight

    document.getElementById("exploration_view").scrollLeft =
        scroll_x * unit + total_length / 2 - screen_width / 2
    document.getElementById("exploration_view").scrollTop =
        scroll_y * unit + total_length / 2 - screen_height / 2
